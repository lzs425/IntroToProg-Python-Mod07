...
# Custom Errors ---------------------------------------------------------------------- #
# class Event_TooShortError(Exception):
#     """  Some custom error info in the DocString  """
#     def __str__(self):
#         return 'Please enter an event at least 25 characters long.'
#Did not end up using custom errors because I did not want to break the program.
# Data ---------------------------------------------------------------------- #
...
class Processor:
    ...
    def add_event_to_date(month, day, new_event, list_of_rows):
        """Writes new event for a month and day based on user input, will replace old event if exists
...
class IO:
    ...
    def print_the_year(list_of_rows):
        ...
        for row in list_of_rows: #need to sort
            print(str(row["Month"]) + "/" + str(row["Day"]) + " - " + str(row["Event"]))
        print("*******************************************")
        print()  # Add an extra line for looks
    ...
    def new_event_request():
        ...
        except ValueError: #likely unnecessary as input() will always turn response into a string, just practicing.
    ...
    def input_new_event(month,day,list_of_rows, new_event_response):
        ...
        while(True):
            ...
            if len(new_event_entry)<25:
                print("The entry was too short, please enter at least 25 characters")
            else:
...
# Main  --------------------------------------------------------------- #
#Processor.save_data_to_file(file_name=file_name_pickle_str, list_of_rows=table_lst) to create file
...
while(True):
    ...
    if choice_str.strip() == '1':#prints the table of data
        ...
        if len(hist_event_str)<3: #Ask for new event input if blank
